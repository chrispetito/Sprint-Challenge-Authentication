...
Sessions can be used to persist authentication so tthe user doesn't need to re-enter credentials for every request the client makes to the server.

1. What does bcrypt do to help us store passwords in a secure manner.

`bcrypt` hashes the passwords so they are not stored in the database as plain text.

1. What does bcrypt do to slow down attackers?

In order to gain access to the password, the attacker would need to have the hash, know the algorithm that was used to hash the password, and know how many rounds were used to generate the hash.

1. What are the three parts of the JSON Web Token?

The three parts of a `JSON Web Token (JWT)` are the header, the payload, and the signature.
